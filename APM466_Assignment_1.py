...
bonds = pd.read_excel("APM466_Assignment_1.xlsx",sheet_name="Price Data 10 Days")
...
for i, row in bonds.iterrows():
    for col in bonds.columns[2:]:  # Start from the second column, excluding "Date"
        ...
        #print(f"Number of periods: {num_periods}")  # Debugging statement
...
# Now bonds_data contains the new values with the Date column unchanged

# Extract Maturity Date (e.g., "CAN 1.25 Mar 25" -> "Mar 2025")
...
# Extract coupon value (Assuming the coupon is in the second column in the format 'X% Bond')
# print(float(spot_rate_data.iloc[0, 0].split()[1].strip()))
...
# Iterate through each bond
for index, row in spot_rate_data.iterrows():
    x_value = row["Maturity Date"]  # The bond's maturity date on x-axis
    y_values = row.drop(["Maturity Date", "Date"]).values.astype(float)  # The bond's yields on y-axis
...
# Copy the bonds_data to forward_rates_data
...
# Iterate through each bond
for index, row in forward_rates_data.iterrows():
    x_value = row["Maturity Date"]  # The bond's maturity date on x-axis
    y_values = row.drop(["Maturity Date", "Date"]).values.astype(float)  # The bond's yields on y-axis
...
def get_covariance_matrix(yield_df,forward_rate_df):
    # Extract yield and forward rates excluding "Date" and "Maturity Date"
    ...
    # Adjust the labels as needed based on your data
